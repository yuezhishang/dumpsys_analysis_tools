...
[![License](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE) [![Platform](https://img.shields.io/badge/platform-Windows%20%7C%20macOS%20%7C%20Linux-lightgrey.svg)](#快速开始) [![Version](https://img.shields.io/badge/version-v1.47-green.svg)](#版本)
...
- 🎯 **聚焦**：只显示**当前聚焦节点**的子树，顶部常驻**面包屑**（`根 → … → 当前`）；点子树内的节点即**下钻**进它的子树，点面包屑任一段即**上溯**。聚焦工具条设有 **Task 下拉选择器**（列出所有 Task 的 `Task #id / 包名 / Activity`），切换即把焦点重置到该 Task 的 `DecorView` 根；视图顶部还显示「当前 Task：…」标识行。适合**沿单条路径深挖某个控件链**，但全局结构较弱。
...
- **v1.46**：**修复 v1.45 工具栏「🌳 大纲 / 🎯 聚焦」切换按钮未渲染**——v1.45 的 JS（`setVtViewMode` / 聚焦渲染）与 `#vt-focus` 容器都已提交，但 `#vt-toolbar` 的 HTML 改造漏提（模式切换按钮与 `vt-controls-*` 包裹层没进文件），导致工具条只显示旧版筛选框、看不到切换按钮。本版补回按钮与 `vt-controls-outline`/`vt-controls-focus` 包裹层，并给校验脚本加「直接读 `index.html` 文本、断言 4 个 id 与按钮文本存在」的硬校验（301 项全绿），杜绝「JS 写了但 HTML 没接」这类回归。徽章此前也漏升到 v1.45，本版一并补到 v1.46。

- **v1.47**：**修复聚焦模式只能看第一个 Task、无法切换**——聚焦模式的子树渲染只认 `vtFocusNodeId`，为 `null` 时永远取 `tasks[0].roots[0]`，而工具条仅「返回根」、面包屑只能在一个 task 树内上下钻，**多 task 时完全进不去第 2/3 个 task**。本版在聚焦工具条加 **Task 下拉选择器**（`vt-focus-task`，列出每个 Task 的 `Task #id / 包名 / Activity`，`onchange` 调 `vtFocusSelectTask` 把焦点重置到该 task 根）+ 视图顶部「当前 Task：…」标识行；新增 `vtFocusSelectTask(idx)` / `vtTaskIndexOfNode(node)` 两个函数，并给校验脚本加「默认停 task0 → 切到 task1 → 切回 task0（含 3-task 样本切到 task2）」断言，**现 306 项全绿**。
